...
export interface Address {
  GetAllAddresses(request: Empty): Promise<AddressBook>;
  GetAddress(request: AddressRequest): Promise<Person>;
}
...
export class AddressClientImpl implements Address {
  ...
  constructor(rpc: Rpc, opts?: { service?: string }) {
    this.service = opts?.service || "address.Address";
    this.rpc = rpc;
  ...
  GetAllAddresses(request: Empty): Promise<AddressBook> {
    ...
    const promise = this.rpc.request(this.service, "GetAllAddresses", data);
    return promise.then((data) => AddressBook.decode(new _m0.Reader(data)));
  }

  GetAddress(request: AddressRequest): Promise<Person> {
    const data = AddressRequest.encode(request).finish();
    const promise = this.rpc.request(this.service, "GetAddress", data);
    return promise.then((data) => Person.decode(new _m0.Reader(data)));
  }
}
